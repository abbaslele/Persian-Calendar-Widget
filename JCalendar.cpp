...
// کلاس استفاده از تاریخ دارای متد های کاربردی
JCalendar::JCalendar(QWidget *i_parent) :
    QWidget(i_parent),
    m_ui(new Ui::JCalendar)
{
    dialog = qobject_cast<QDialog*>(i_parent);
...
"{"
"image: url(:/Resources/Icons/Down_Arrow_Icon_O_E.svg);"
"width: 24px;"
"height: 24px;"
"}"
...
"}";
    ...
    m_ui->moon->setStyleSheet(moonStyle);
}
...
bool JCalendar::isLeap(int i_year) // بررسی سال کبیسه
{
    int mod;
    mod = i_year % 33;
...
bool JCalendar::isValid(Date &i_date) // بررسی معتبر بودن تاریخ
{
...
bool JCalendar::isValid(const QString &i_date) // بررسی معتبر بودن تاریخ با ورودی رشته
{
...
JCalendar::Date JCalendar::toJalali(Date &i_date) // تبیدل تاریخ به جلالی
{
    JCalendar::Date date;
...
JCalendar::Date JCalendar::toGregorian(Date &i_date) // تبدیل تاریخ به میلادی
{
    JCalendar::Date date;
    //set Date
    int Year;
...
int JCalendar::getDayCount(int i_year) // دریافت تعداد روز ها
{
    if( isLeap( i_year ) )
        return 6 * 31 + 6 * 30;
    return 6 * 31 + 5 * 30 + 29;
...
int JCalendar::getDayCount(int i_year, int i_moon) // دریافت تعداد روز ها با ورودی سال و ماه
{
...
int JCalendar::sumOfDaysJalali(Date &i_date) // جمع روز های تاریخ جلالی
{
    QList< int > jDaysInMonth ;
    jDaysInMonth << 31 << 31 << 31 << 31 << 31 << 31 << 30 << 30 << 30 << 30 << 30 << 29;
...
int JCalendar::sumOfDaysGregorian(Date &i_date) // جمع روز های تاریخ میلادی
{
    QList< int > gDaysInMonth;
    gDaysInMonth << 31 << 28 << 31 << 30 << 31 << 30 << 31 << 31 << 30 << 31 << 30 << 31;
    int sum = i_date.Day - 1;
...
JCalendar::Date JCalendar::today() // برگرداندن تاریخ امروز
{
    Date date;
...
int JCalendar::getDayOfWeek(JCalendar::Date &i_date) // بازگرداندن روز چندم هفته
{
...
QString JCalendar::getDayOfWeekName(JCalendar::Date &i_date) // بازگرداندن نام روز هفته
{
...
QString JCalendar::getDayOfWeekNameSmall(JCalendar::Date &i_date) // بازگرداندن نام روز هفته بصورت اختصاری
{
    int dayOfWeek = getDayOfWeek( i_date );
    if( dayOfWeek == 1 )
...
int JCalendar::getDiffInDay(JCalendar::Date &i_from, JCalendar::Date &i_to) // بازگرداندن میزان اختلاف در تعداد روز
{
    Q_UNUSED( i_from );
    Q_UNUSED( i_to );
    return 0;
...
JCalendar::Date &JCalendar::getCurrentDate() // بازگرداندن تاریخ جاری
{
    return m_currentDate;
}

void JCalendar::setDate(Date i_date) // تنظیم تاریخ مورد نظر
{
    ...
    // QString persianNumber = persianLocale.toString(static_cast<int>(i_date.Year)); // Explicit cast to int
    // persianNumber.remove(persianLocale.groupSeparator()); // Remove grouping separators
    m_ui->year->setValue(i_date.Year);
    //m_ui->year->setValue(DeviceManager::getInstance()->toPersianNumbers(QString::number(1400)).toInt());
    //m_ui->year->setValue(DeviceManager::getInstance()->toPersianNumbers(QString::number(i_date.Year)).toInt());
    ...
    m_ui->TextDate->setText(toPersianNumbers(i_date.toString()) );
...
void JCalendar::on_moon_currentIndexChanged(int i_index) // اجرای کدها پس از تغییر اندیس ماه
{
...
void JCalendar::on_year_valueChanged(int i_value) // اجرا متد پس از تغییر سال
{
    Date date = m_currentDate;
    date.Year = i_value;
    if( isValid( date ) )
...
void JCalendar::on_today_clicked() // کدهای دکمه امروز
{
    setDate( today() );
}

QString JCalendar::toPersianNumbers(const QString &input) // تبدیل اعداد به فارسی
{
    QString result;
    for (QChar c : input) {
        if (c.isDigit()) {
...
QString toPersianNumber( int i_val ) // تبدیل به عدد فارسی
{
    QString out;
...
{
    return mCancelButtonClicked;
}

void JCalendar::setCancelButtonClicked(bool CancelButtonClickedInp)
{
    mCancelButtonClicked = CancelButtonClickedInp;
}

bool JCalendar::getAcceptButtonClicked() const
{
    return mAcceptButtonClicked;
}

void JCalendar::setAcceptButtonClicked(bool AcceptButtonClickedInp)
{
    mAcceptButtonClicked = AcceptButtonClickedInp;
}

QString JCalendar::Date::toString() // تبدیل تاریخ به رشته
{
    //return QString("%1-%2-%3").arg( Year ).arg( Month ).arg( Day );

    return  QString("%1-%2-%3").arg(Year, 4).arg(Month, 2, 10, QChar('0')).arg(Day, 2, 10, QChar('0'));
}

QString JCalendar::Date::toStringFull() // تبدیل تاریخ به رشته کامل
{
    return QString("%1-%2-%3").arg( Year ).arg( Month ).arg( Day );
}

QDate JCalendar::Date::toQDate() // تبدیل به تاریخ qt
{
    return QDate( Year, Month, Day );
}

bool JCalendar::Date::operator ==(JCalendar::Date &i_other) // بازتعریف عملگر ==
{
    return Year == i_other.Year && Month == i_other.Month && Day == i_other.Day;
}

JCalendar::Date JCalendar::Date::fromQDate(const QDate &i_qDate) // تبدیل از تاریخ qt
{
    Date date;
...
JCalendar::Date JCalendar::Date::fromString(const QString &i_date) // تبدیل از رشته
{
    Date date;
...
JCalendar::Date JCalendar::Date::fromString(const QString &i_date, const QString &i_format) // تبدیل از رشته با ورودی تاریخ و فرمت
{
    Date date;
...
void JCalendar::on_TextDate_editingFinished() // اجرا پس از پایان تغییر تاریخ در فیلد
{
...
void JCalendar::on_uCancel_PushButton_clicked() // کدهای دکمه cancel
{
...
void JCalendar::on_uAccept_PushButton_clicked() // کدهای دکمه accept
{
